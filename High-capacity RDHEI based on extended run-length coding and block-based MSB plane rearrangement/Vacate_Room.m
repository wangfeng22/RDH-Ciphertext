function [vacate_I,num_LSB] = Vacate_Room(origin_I,Block_size,L_fix)
% 函数说明：利用BMPR算法压缩原始图像，以空出可以嵌入秘密信息的空间
% 输入：origin_I（原始图像）,Block_size（分块大小）,L_fix（定长编码参数）
% 输出：vacate_I（空出空间的图像）,num_LSB（空出的总空间大小）
...
for k=1:num_pl
    ...
    Trans_Plane = reshape(PL_bits,row,col); %换成矩阵,以列排序
    Plane = Trans_Plane'; %矩阵转置
%     Plane = zeros(row,col); %最终位平面矩阵
%     t = 0; %计数
%     for i=1:row
%         for j=1:col
%             t = t+1;
