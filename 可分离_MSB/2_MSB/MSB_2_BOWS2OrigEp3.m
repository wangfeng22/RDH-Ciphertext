...
%% 记录每张图像的嵌入量
num_BOWS2OrigEp3 = zeros(1,img_num); %嵌入量
bpp_BOWS2OrigEp3 = zeros(1,img_num); %嵌入率
%% 图像数据集测试
for i=1:img_num
    ...
    %---------------结果记录----------------%
    [m,n] = size(origin_I);
    num_BOWS2OrigEp3(i) = numData;
    bpp_BOWS2OrigEp3(i) = numData/(m*n);
    %---------------结果判断----------------%
